...
public class BraveSyncScreensPreference extends BravePreferenceFragment
    ...
    private static String mDefaultSyncServiceUrl = "";
    // Custom Sync service url
    private static String mCustomSyncServiceUrl = "";
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mDefaultSyncServiceUrl = getBraveSyncWorker().getDefaultSyncServiceURL();
    ...
    private void showAdvancedOptionsDialog() {
        ...
        mCustomSyncServiceUrl = getBraveSyncWorker().getCustomSyncServiceURL();
        ...
        if (mSyncUrlResetToDefaultButton != null) {
            ...
            mSyncUrlResetToDefaultButton.setOnClickListener(
        ...
        if (mCustomSyncServiceUrl.isEmpty()) {
            mCustomSyncUrlInput.setHint(mDefaultSyncServiceUrl);
            mSyncUrlResetToDefaultButton.setEnabled(false);
            mSyncUrlResetToDefaultButton.setTextColor(
                    getContext().getColor(R.color.brave_sync_disabled_button_color));
        } else {
            mCustomSyncUrlInput.setText(mCustomSyncServiceUrl);
        }

        mSaveAndRelaunchButton = view.findViewById(R.id.brave_sync_save_and_relaunch);
        if (mSaveAndRelaunchButton != null) {
            mSaveAndRelaunchButton.setOnClickListener(
                    new View.OnClickListener() {

                        @Override
                        public void onClick(View v) {
                            saveCustomSyncUrlAndRelaunch();
                        }
                    });
        }
        ...
        mCustomSyncUrlInput.addTextChangedListener(
                new TextWatcher() {
                    ...
                    public void onTextChanged(CharSequence text, int start, int before, int count) {
                        // Enable Save and relaunch button
                        if (text.length() > 0) {
                            mSyncUrlResetToDefaultButton.setEnabled(true);
                            mSyncUrlResetToDefaultButton.setTextColor(
                                    getContext().getColor(R.color.leo_reset_text_color));
                        } else {
                            ...
                            mSyncUrlResetToDefaultButton.setTextColor(
                                    getContext()
                                            .getColor(R.color.brave_sync_disabled_button_color));
                        }

                        if (!text.toString().equals(mCustomSyncServiceUrl)) {
                            mSaveAndRelaunchButton.setEnabled(true);
                        } else {
                            mSaveAndRelaunchButton.setEnabled(false);
                        }
                    }
                });

        Button cancelButton = view.findViewById(R.id.brave_sync_advanced_options_cancel_btn);
    ...
    private void saveCustomSyncUrlAndRelaunch() {
        ...
        if (url.isEmpty() || Patterns.WEB_URL.matcher(url).matches()) {
            getBraveSyncWorker().setCustomSyncServiceURL(mCustomSyncUrlInput.getText().toString());
            BraveRelaunchUtils.restart();
        } else {
            mCustomSyncUrlInput.setError(
                    getActivity().getResources().getString(R.string.invalid_url));
        }
    }

    private void resetCustomSyncUrl() {
        // Show confirmation prompt to reset.
        mCustomSyncUrlInput.setText("");
        getBraveSyncWorker().setCustomSyncServiceURL("");
    }
